...
public class JoueurIA extends AbstractJoueur {
    ...
    /** Renvoie la couleur qu'il choisi pour le deuxi̬me tour d'atout (ou null) */
    ...
    private Carte donneUneGrosseCarteA(CouleurCarte couleur) {
        ...
        for ( Carte c : carteEnMain)
            if ( c.getCouleur().equals(couleur))
                if ( (meilleur == null) || (analyseur.positionDe(meilleur)<gestionnaireCarte.positionDe(c)))
                    meilleur = c;
    ...
    /** Retourne VRAI si seulement un des deux adversaires poss̬de autant
    ...
    private Carte donneUneQuiPrendUneGrosseDAtout() {
        if ( ilOntDesAtouts() ) {
            ...
            Carte aPrendre = null;
            int nombre = 0;
            for ( Carte c : gestionnaireCarte.cartesNonJouees)
                if ( c.estCouleur(atout) && ! carteEnMain.contains(c) &&
                     (gestionnaireCarte.positionDe(c)<gestionnaireCarte.positionDe(meilleur)) &&
                     (regle.pointsDe(c) >= 10)) {
